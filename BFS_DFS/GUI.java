...
public class GUI extends JFrame implements ActionListener {
	...
	private final String author = "Lê Hồng Duy - PTIT";
	...
	private JTextField tfNumberPoint, tfBeginPoint, tfEndPoint, tfStartPoint,tfendPoint;
	...
	private JPanel createControlPanel() {
		...
		// create edge panel - select algorithm
		JPanel runPanel = new JPanel(new GridLayout(5, 2, 5, 5));
...
//		runPanel.add(new JLabel(""));
		runPanel.add(new JLabel("Điểm kết thúc:"));
		runPanel.add(tfendPoint = createTextField());
		runPanel.add(new JLabel(""));
		runPanel.add(btnRun = createButton("Duyệt"));
		runPanel.add(new JLabel("Đường đi"));
		runPanel.add(res = createTextField());
	...
	private void run() {
		...
		int endPoint = getValue(tfendPoint,graphicsPanel.getNumberPoint());
		if (startPoint > 0) {
			startPoint--;
			graph.initVisit();
			graph.initListVisit();
			if (radBFS.isSelected()) {
				...
				res.setText(graph.toStringListPoint(1,endPoint));
			} else {
